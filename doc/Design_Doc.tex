...
\section{Implementation}
    ...
    \subsection{Design Logic}
        ...
        \begin{tikzpicture}[
            ...
            \draw[arr, bend right=20] (idle)    to
                node[lbl, right]{WASD}    (orbmove);
            \draw[arr, bend right=20] (orbmove) to
                node[lbl, left] {release} (idle);

            \draw[arr, bend left=15] (orbmove) to
                node[lbl, above left]{pressure\\plate} (blocker);
            \draw[arr, bend left=15] (blocker) to
                node[lbl, below right]{stop\\reached}  (orbmove);
